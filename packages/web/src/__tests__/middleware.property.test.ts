import * as fc from 'fast-check'; // Reordered
import { SignJWT, jwtVerify } from 'jose';
import { describe, it, expect } from 'vitest'; // Removed beforeEach, reordered
...
async function validateToken(
  token: string,
): Promise<{ valid: boolean; payload?: unknown; error?: string }> { // Changed any to unknown
  try {
...
describe('JWT Validation Middleware - Property Tests', () => {
  /**
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (payload) => {
          ...
          const tamperedToken = `${validToken}tampered`; // Used template literal
          ...
          if (parts.length === 3) {
            const wrongSignature = `${parts[0]}.${parts[1]}.wrongsignature`; // Used template literal
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.oneof(
          ...
          fc.constant('a.b.c.d'), // Too many parts // Added trailing comma
        ),
